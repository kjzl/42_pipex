...
/*   Updated: 2024/06/15 07:26:47 by kwurster         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	cmd_set_file_out(t_cmd *cmd, const char *file_out)
{
	cmd->out[1] = open(file_out, O_WRONLY | O_CREAT | O_TRUNC,
		...
		char *const *envp, t_cmd *out)
{
	...
	if (!cmd_set_file_out(out, file_out))
	{
		close_pipe(out->in);
		return (false);
	}
